...
for i in range(3):
    print('subset '+label[i]+' :', file=flog)
    ra, de = np.take(RAg, ind[i]),  np.take(DEg,  ind[i])
    ua, ud = np.take(pmRA, ind[i]), np.take(pmDE, ind[i])
    eu = np.ones(ua.size)
    
    print('Weighted: Normal.', file=flog)
    w, sig, corrcoef = RotationFit(ua, ud, eu, eu, ra, de)
    [wx,   wy,  wz] = w
    [ewx, ewy, ewz] = sig

    print('the spin are(uas*yr-1):\n :\n', \
        '    &$%.3f \pm %.3f$'*3%(wx, ewx, wy, ewy, wz, ewz), file=flog)
    print('   correlation coefficients are:\n', corrcoef, file=flog)
    print('from J2000.0-2015.0, the cumulative orientation are(uas):\n :\n', \
        '    &$%.3f \pm %.3f$'*3%(wx*15, ewx*15, wy*15, ewy*15, wz*15, ewz*15), file=flog)
...
print('the orientation/spin are(uas*yr-1):\n :\n', \
        '    &$%.3f \pm %.3f$'*3%(wx, ewx, wy, ewy, wz, ewz), file=flog)
print('   correlation coefficients are:\n', corrcoef, file=flog)
print('from J2000.0-2015.0, the cumulative orientation are(uas):\n :\n', \
        '    &$%.3f \pm %.3f$'*3%(wx*15, ewx*15, wy*15, ewy*15, wz*15, ewz*15), file=flog)
