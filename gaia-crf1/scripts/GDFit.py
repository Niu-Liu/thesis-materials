...
#    First, no weight apply to the data. Calculate the w.r.m.s' '''
#   
...
#    print>>flog, 'Next, apply the weigths of 1.0/M_err**2'
...
print(time.strftime('\nThe scripts runs at %Y-%m-%d %H:%M:%S',time.localtime(time.time())), file=flog)
...
## set a 100% barrier for plx_err/plx ratio
bar = 50
...
print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())), file=flog)
