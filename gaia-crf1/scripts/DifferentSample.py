...
## generate the new array after eliminating outliers 
    ...
    print('Eliminating(%2.1f-sigma): %d/%d (objects/Outliers).'%(n,len(pml), Num2), file=flog)
## final result.
    '''
    ...
    print('''
    After elimination: ''', file=flog)
    print('The parameters S1, S2, S3(km/s):\n', \
        '    %.2f \pm %.2f'*3%(S1, eS1, S2, eS2, S3, eS3), file=flog)
    print(' Oort constants A, B(km/s/kpc) :\n ',  \
        '    %.2f \pm %.2f'*2%(A, eA, B, eB), file=flog)
    print(' Oort constants A, B(mas/yr) :\n ',  \
        '    %.3f \pm %.3f'*2%(A/k, eA/k, B/k, eB/k), file=flog)
    print('   correlation coefficients are:\n', corrcoef, file=flog)
    
    V0 = (A-B)*R0
    eV0 = R0*np.sqrt(eA**2 + eB**2)
    print('rotational velocity: %.2f \pm %.2f km/s.\n'%(V0, eV0), file=flog)
## used in latex.    
    print('&%d/%d'%(len(pml), Num2) + \
        '\t&$%.2f \pm %.2f$'*3%(S1, eS1, S2, eS2, S3, eS3) + \
        '\t&$%.2f \pm %.2f$'*2%(A, eA, B, eB) + \
        '\t&$%.2f \pm %.2f$'%(V0, eV0), file=flog1)
...
## final result. 
    ...
    print('The parameters S1, S2, S3(km/s):\n', \
        '    %.2f \pm %.2f'*3%(S1, eS1, S2, eS2, S3, eS3), file=flog)
    print(' Rotation D32M, D13M, D21M (km/s/kpc) :\n ',  \
        '    %.2f \pm %.2f'*3%(D32M, eD32M, D13M, eD13M, D21M, eD21M), file=flog)
    print(' Shear D12P, D13P, D32P (km/s/kpc) :\n ',  \
        '    %.2f \pm %.2f'*3%(D12P, eD12P, D13P, eD13P, D32P, eD32P), file=flog)   
    print(' Rotation D32M, D13M, D21M (mas/yr) :\n ',  \
        '    %.3f \pm %.3f'*3%(D32M/k, eD32M/k, D13M/k, eD13M/k, D21M/k, eD21M/k), file=flog)
    print(' Shear D12P, D13P, D32P (mas/yr) :\n ',  \
        '    %.3f \pm %.3f'*3%(D12P/k, eD12P/k, D13P/k, eD13P/k, D32P/k, eD32P/k), file=flog)    
    print('   correlation coefficients are:\n', corrcoef, file=flog)
...
#        '\t&$%.2f \pm %.2f$'%(V0, eV0) 
    ...
    print('&%d/%d'%(len(pml), Num2) + \
        '##val'+'  \t&$%.2f $'*10%(S1, S2, S3, D32M, D13M, D21M, D12P, D13P, D32P, V0), file=flog1) 
    print('##err'+'  \t&$\pm %.2f$'*10%(eS1, eS2, eS3, eD32M, eD13M, eD21M,\
                                      eD12P, eD13P, eD32P, eV0), file=flog1) 
...
## final result. 
    ...
    print('The parameters S1, S2, S3(km/s):\n', \
        '    %.2f \pm %.2f'*3%(S1, eS1, S2, eS2, S3, eS3), file=flog)
    print(' Rotation D32M, D13M, D21M (km/s/kpc) :\n ',  \
        '    %.2f \pm %.2f'*3%(0, 0, D13M, eD13M, D21M, eD21M), file=flog)
    print(' Shear D12P, D13P, D32P (km/s/kpc) :\n ',  \
        '    %.2f \pm %.2f'*3%(D12P, eD12P, D13P, eD13P, 0, 0), file=flog)   
    print(' Rotation D32M, D13M, D21M (mas/yr) :\n ',  \
        '    %.2f \pm %.2f'*3%(0/k, 0/k, D13M/k, eD13M/k, D21M/k, eD21M/k), file=flog)
    print(' Shear D12P, D13P, D32P (mas/yr) :\n ',  \
        '    %.2f \pm %.2f'*3%(D12P/k, eD12P/k, D13P/k, eD13P/k, 0/k, 0/k), file=flog)    
    print('   correlation coefficients are:\n', corrcoef, file=flog)
...
#        '\t&$%.2f \pm %.2f$'%(V0, eV0)
    print('&%d/%d'%(len(pml), Num2) + \
        '##val'+'  \t&$%.2f $'*8%(S1, S2, S3, D13M, D21M, D12P, D13P, V0), file=flog1) 
    print('##err'+'  \t&$\pm %.2f$'*8%(eS1, eS2, eS3, eD13M, eD21M,\
                                      eD12P, eD13P, eV0), file=flog1) 
...
print('''#############################################################
## Using TGAS catalog:''', file=flog)
print(time.strftime('##The scripts runs at %Y-%m-%d %H:%M:%S\n',time.localtime(time.time())), file=flog)
flog1= open('../logs/GDfit.tex', 'a')
print(time.strftime('#The scripts runs at %Y-%m-%d %H:%M:%S',time.localtime(time.time())), file=flog1)
print('## result that can be used in latex.', file=flog1)
...
bar = 30
#bar = 1e10
print('## barrier: %d '%bar, file=flog)
print('## barrier: %d '%bar, file=flog1)
...
err = 0.20
plx = plx*(1+err)
print(' ##plx = plx *( 1+ %.2f):'%err, file=flog1)
print(' ##plx = plx *( 1+ %.2f):'%err, file=flog)
...
#    raGs, decGs, ls)
    ...
    print("## %.1f kpc < r < %.1f kpc: TGAS"%(rl[i],ru), file=flog)
    print("## %.1f kpc < r < %.1f kpc: TGAS"%(rl[i],ru), file=flog1)
