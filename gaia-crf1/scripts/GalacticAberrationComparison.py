...
for i in range(G.size/3):
    gE = G[i]
    print('## The Glide in equatorial coordinates (mas):\n', gE, file=flog)
    A = np.sqrt(np.sum(gE**2))/13.7*1000 ## uas/yr
    print('## magnitude (uas/yr):\n', A, file=flog)
    gG = np.array(Eq2Ga_rect(gE)).reshape(3,)
    print('## The Glide in galactic coordinates (mas):\n', gG, file=flog)
    l, b = cord2arc(gG)
    print('The dipole apex in galactic coordinates (l,b): (%.2f, %.2f)\n'%(np.rad2deg(l), np.rad2deg(b)), file=flog)