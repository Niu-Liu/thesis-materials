...
def crsid_TT(s):
    ...
    print(''' Cross identification between TGAS and Tycho-2 begins.''', file=flog)
...
#    print indice1[0], indice2[0]
    ...
    ra3, dec3, pmra3, pmdec3, ra_err3, dec_err3, pmra_err3, pmdec_err3 = \
    np.loadtxt(fty2, usecols=(list(range(1,9))), unpack=True)
    
    ra4, ra_err4, dec4, dec_err4, plx, plx_err, \
    pmra4, pmra_err4, pmdec4, pmdec_err4, l, b , g_mag\
    = np.loadtxt(ftgas, usecols=(list(range(1,11))+[21,22,23]), unpack=True)
#    print ra3[i]
#    print ra4[j]
...
## header information of FITS      
    ...
    print(s+': %d/%d entries are found. Completed!'%(num, N), file=flog)
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())), file=flog)
    flog.close()
